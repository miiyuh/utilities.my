...
export const DEFAULT_TILED: TiledOptions = { opacity: 0.25, angle: -30, fontSize: 28, spacing: 2.2 }
...
export function loadPersisted(): WatermarkOptions {
  try {
    ...
    if (raw) {
      ...
      const t: Partial<TiledOptions> = o.tiled ?? {}
      // Pick known keys rather than spreading, so settings saved by an older
      // build (which had a band, a card layout and a QR shield) do not carry
      // dead fields forward for ever. The next save writes the clean shape.
      return {
        ...DEFAULT_OPTIONS,
        presetId: o.presetId ?? DEFAULT_OPTIONS.presetId,
        customLines: o.customLines ?? DEFAULT_OPTIONS.customLines,
        ...
        tiled: {
          ...
          angle: clamp(Number.isFinite(t.angle) ? Number(t.angle) : DEFAULT_TILED.angle, -60, 60),
          fontSize: clamp(Number(t.fontSize) || DEFAULT_TILED.fontSize, 16, 48),
          spacing: clamp(Number(t.spacing) || DEFAULT_TILED.spacing, 1.5, 4),
        },
      }
    }
  } catch (error) {
...
/**
 * Render one side: the cropped card with the tiled watermark over it.
 */
export function renderWatermarkedCard(
  ...
  drawTiled(ctx, target.w, target.h, tiledText(resolveLines(options)), options.tiled, options.color)
  return canvas
}
